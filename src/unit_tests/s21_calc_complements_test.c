...
START_TEST(calc_complemets_test_1) {
  matrix_t m = {0};
  matrix_t result = {0};
  ...
  if (codec == 0) {
    ...
    ck_assert_double_eq_tol(result.matrix[0][0], 1, 1e-6);
    s21_remove_matrix(&m);
...
START_TEST(calc_complemets_test_2) {
  matrix_t m = {0};
  matrix_t result = {0};
  int code = s21_calc_complements(&m, &result);
...
START_TEST(calc_complemets_test_3) {
  matrix_t m = {0};
  matrix_t result = {0};
...
START_TEST(calc_complemets_test_4) {
  matrix_t expected = {0};
  matrix_t m = {0};
  matrix_t result = {0};
...
Suite *suite_complements_test() {
  ...
  tcase_add_test(tc, calc_complemets_test_1);
  tcase_add_test(tc, calc_complemets_test_2);
  tcase_add_test(tc, calc_complemets_test_3);
  tcase_add_test(tc, calc_complemets_test_4);
